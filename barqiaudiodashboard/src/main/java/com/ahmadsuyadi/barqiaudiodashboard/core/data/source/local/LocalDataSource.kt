...
class LocalDataSource(
    ...
    suspend fun deleteFavoriteByIdAudio(idFavoriteAudio: Int) =
        audioDao.deleteFavoriteByIdAudio(idFavoriteAudio)

    fun getFavoriteByIdAudio(idFavoriteAudio: String) =
        audioDao.getFavoriteByIdAudio(idFavoriteAudio)
    ...
    suspend fun updateDownloadedByReq(reqDownload: Long) =
        audioDao.updateDownloadedByReq(reqDownload)

    suspend fun deleteDownloadedByIdAudio(idDownloadedAudio: String) =
        audioDao.deleteDownloadedByIdAudio(idDownloadedAudio)

    fun getListDownloaded() = audioDao.getListDownloaded()
    fun getDownloadByIdAudio(idDownload: String) = audioDao.getDownloadByIdAudio(idDownload)
    ...
    suspend fun deletePlaylist(playlistEntity: PlaylistEntity) =
        audioDao.deletePlaylist(playlistEntity)
    ...
    suspend fun deleteAudioFromPlaylist(data: PlaylistAudioCrossRef) =
        audioDao.deleteAudioFromPlaylist(data)

    suspend fun deleteAllAudioByPlaylist(playlistId: Int) =
        audioDao.deleteAllAudioByPlaylist(playlistId)
